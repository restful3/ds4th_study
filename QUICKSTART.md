...
먼저 자료와 일정을 확인하고 구성 계획을 제안한 다음 상세 HTML 리포트와 HTML 발표자료를 만들어줘.
```
...
새 회차는 별도 지시가 없으면 Chapter 1 완성본을 기준으로 `study-report-v1` 상세 리포트와 `study-deck-v1` 발표자료를 함께 생성한다. 리포트에는 자동 목차·테마·인쇄·전체화면 이미지 줌/팬이, 발표자료에는 18–26장 기본 흐름·키보드 이동·자동 목차·진행률·전체화면 기능이 포함된다.
