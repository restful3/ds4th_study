...
![Checkpoints](images/checkpoints.avif)
...
The state of a thread at a particular point in time is called a checkpoint. Checkpoint is a snapshot of the graph state saved at each super-step and is represented by a `StateSnapshot` object with the following key properties:
...
2.  **Input checkpoint** with user input `{'foo': '', 'bar': []}` and `node_a` as next.
3.  **Intermediate checkpoint** with node_a outputs `{'foo': 'a', 'bar': ['a']}` and `node_b` as next.
4.  **Final checkpoint** with node_b outputs `{'foo': 'b', 'bar': ['a', 'b']}` and no next nodes.

*Note: The `bar` channel contains combined outputs because it uses a reducer.*

---

## Get state

When interacting with the saved graph state, you **must** specify a [thread identifier](#threads). You can view the **latest** state by calling `graph.get_state(config)`. This returns a `StateSnapshot` object that corresponds to the latest checkpoint associated with the thread ID provided in the config or a checkpoint associated with a checkpoint ID for the thread, if provided.
...
StateSnapshot(
    ...
    parent_config={'configurable': {'thread_id': '1', 'checkpoint_id': '1ef663ba-28f9-6ec4-8001-31981c2c39f8'}},
    tasks=()
)
```
...
![State history diagram](images/checkpoints.avif)

## Replay

It’s also possible to play-back a prior graph execution. If we [invoke](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.invoke) a graph with a `thread_id` and a `checkpoint_id`, then we will *re-play* the previously executed steps *before* a checkpoint that corresponds to the `checkpoint_id`, and only execute the steps *after* the checkpoint.
...
The final thing you can optionally specify when calling `update_state` is `as_node`. If you provided it, the update will be applied as if it came from node `as_node`. If `as_node` is not provided, it will be set to the last node that updated the state, if not ambiguous. The reason this matters is that the next steps to execute depend on the last node to have given an update, so this can be used to control which node executes next. See this [how to guide on time-travel](/oss/python/langgraph/use-time-travel) to learn more about forking state.

![Update State](images/checkpoints_full_story.avif)
...
![Model of shared state](images/shared_state.avif)
...
# Let's say hi again
for update in graph.stream(
    {"messages": [{"role": "user", "content": "hi, tell me about my memories"}]}, config, stream_mode
):
    print(update)
```
