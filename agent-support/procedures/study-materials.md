...
책 코드를 실제로 실행하면 오류가 계속 드러난다. 이 교재에서만 수십 건이다 — 인쇄된 코드가
...
- **그림 안 영문을 번역할 때는 원본을 덮어쓰지 않는다.** `localize-image-text` 스킬을 쓰고,
  번역 PNG와 재현 가능한 overlay spec을 각각 `images/ko/`와 `images/ko/specs/`에 둔다.
  파일명은 `figure-<장>-<그림>-ko.png`로 통일하고 한국어 문서만 그 파일을 참조한다. 원서와
  원문 문서는 원본 해시 파일을 계속 참조한다.

  장마다 `images/ko/manifest.json`에 원본↔출력 대응, 번역/보존 정책, 검수 상태를 기록한다.
  완료 조건은 원본과 출력의 픽셀 크기가 같고, 두 한국어 Markdown의 그림 참조가 모두
  존재하며, 원본 크기와 실제 문서 표시 크기에서 렌더를 직접 확인한 상태다. 회색 비활성 라벨,
  회전 글자, 캔버스 가장자리의 잘린 영문 조각도 검수 대상이다.

  다만 이 저장소의 공개 정책을 먼저 확인한다. **권리 확인 없는 책 그림·표 스캔과 그에 가까운
  번역 파생 이미지는 public git에 새로 추가하지 않는다.** 로컬 학습용 생성과 공개 저장소
  게시 승인은 별개의 단계다.
...
경계가 헷갈리기 쉬우므로 규칙 둘을 못박는다.

- **무엇이 바뀌어야 `substituted` 인가.** 그 리스팅이 **직접 부르는** 의존물을 런타임에
  갈아 끼웠을 때만이다 (책 15장의 15.2 가 `jinja2_formatter` 를 그렇게 한다). 모듈이
  임포트되게 하려고 자리표시자 환경변수를 채우거나 모델 객체를 바꾸는 것은 그 리스팅이
  **받는 설정** 이 달라지는 것이지 코드가 달라지는 것이 아니다 — `original` 이다.
- **어느 실행 경로를 기준으로 세는가 — 최악 경로.** 노트북이 제공하는 실행 모드 중
  **하나라도** 직접 의존물이 교체되면 `substituted` 다. `run` 이 `executed` 인 기본
  경로에서는 그 코드가 안 불리고 서비스가 있어야 도는 경로에서만 불리더라도 그렇다.
  기본 경로만 보면 "서비스를 갖춘 스터디원이 실제로 겪는 것" 이 표에서 사라진다.
  대신 `note` 에 **어느 경로에서 교체가 걸리는지** 를 반드시 적는다.

한 축 문자열은 두 축의 **줄임말** 이고, 게이트의 `verify.coverage_axes()` 가 그 변환을
정의한다. 검증도 그 변환을 거쳐서 하므로 두 표기가 섞여 있어도 판정이 갈리지 않는다.
...
**표로 바꾸는 경우는 둘이다.** ① 줄임말로 표현되지 않는 축 조합일 때, ② 조합 자체는
줄임말로 되지만 **왜 그런지 남겨야 할 때**(`note`). 15.2 는 ②다 — `executed/substituted`
는 `substituted` 로도 적히지만, 무엇을 어느 경로에서 갈아 끼웠는지가 그 판정의 근거라서
표로 둔다.
