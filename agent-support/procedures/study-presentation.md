...
- `agent-support/templates/STUDY_SESSION_BLUEPRINT.md`와 그 문서가 가리키는 Chapter 1 완성본을 기준으로 학습 목표, 핵심 흐름, 데모 필요 여부, 리포트 목차와 예상 슬라이드 구성을 함께 설계한다.
- 기본 논리는 `문제 → 개념 → 메커니즘 → 운영 구조 → 근거와 한계 → 사례 → 판단 → 요약`이다. 교재에 맞게 합칠 수는 있지만 빠진 축이 없는지 확인한다.
- 50분 발표는 보통 18–26장으로 구성하고, 처음 3장은 표지·핵심 질문·전체 흐름, 마지막 2장은 확인 질문·세 문장 요약으로 둔다.
